...
class DBfunctions implements iDBfunctions{
    private $valueBag = [];
    private $table = '';
    private $db;
    private string $sql = '';
...
public function whereOr(): object{
    ...
    private function _runGetQuery($getMethod): array{
        ...
        if($getMethod === 'DBfunctions::get')
           $recordset = $dbStatement->fetch(PDO::FETCH_BOTH); 
        elseif($getMethod === 'DBfunctions::getAll')   
    ...
    private function _runWhere($field, $operator, $value, $whereMethod): void{
        ...
        if($this->whereInstanceCounter > 0){
            if($whereMethod === 'DBfunctions::where')
                $this->sql .= ' and ';
            elseif($whereMethod === 'DBfunctions::whereOr')
                $this->sql .= ' or ';
    ...
    private function _executeQuery(): int{
        try {
            ...
            $numberValueBag = count($this->valueBag); 
             
            while($valueCounter < $numberValueBag){
